...
// システム全体のX API設定画面
// - X API クライアントキー入力フィールド
// - X API シークレットキー入力フィールド  
// - コールバックURL表示（コピー用）

// static/admin-x-notification.js
// 管理画面のクライアントサイドロジック
// - APIキー・シークレットキーの保存/読み込み
// - キーの暗号化保存処理
```

### コミュニティ編集画面UI（コミュニティオーナー用）

```javascript
// templates/community/x-settings-modal.tpl
// コミュニティごとのX通知設定モーダル
// - 「Xアカウントを接続」ボタン（OAuth開始）
// - 接続済みアカウント一覧（@username表示）
// - アカウント選択/削除/再接続
// - イベントタイプごとの有効/無効チェックボックス
// - テンプレートカスタマイズテキストエリア
// - テスト投稿ボタン

// static/community/x-settings.js
// コミュニティ編集画面のクライアントサイドロジック
// - OAuth接続フロー開始（ポップアップウィンドウ）
// - コールバック受け取り処理
// - アカウント管理（選択・削除）
// - 通知設定の保存/読み込み
// - テスト投稿実行
