...
- 05: 投稿時にAIでモデレーションを行う。しきい値を超える場合、要モデレーションのフラグを立てる
  - [x] 専用プラグインの作成
  - [ ] フィルターの設定（トピック作成、コメント作成、返信）。モデレーションはまだ実装しない
  - [ ] システム管理者用設定画面の作成（APIキーの入力、デフォルト値のしきい値の設定）
  - [ ] モデレーションの実装（判定結果の保存、通知機能の実装）
  - [ ] フラグの立った投稿をマネージャー、オーナーが確認できるようにする（投稿内容、同一ユーザーによる過去の拒否回数の表示）
  - [ ] 投稿の判定（許可、拒否）
  - [ ] ユーザーをバンする機能の実装
